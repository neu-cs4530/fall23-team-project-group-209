...
describe('[] UNOAreaController', () => {
  ...
  describe('drawDeck', () => {
    it('should return the draw deck', () => {
        ...
        const controller = UNOAreaControllerWithProp({
            status: 'IN_PROGRESS', 
            players: [ourUNOPlayer, otherUNOPlayer],
            deck: [{color: 'Blue', rank: 1}, {color: 'Red', rank: 2}],
        });
        expect(controller.drawDeck).toEqual([{color: 'Blue', rank: 1}, {color: 'Red', rank: 2}]);
    });
  ...
  describe('ourDeck', () => {
    it('should return deck of our player', () => {
        const ourUNOPlayer: UNOPlayer = {cards: [{color: 'Blue', rank: 1}, {color: 'Red', rank: 2}], id: ourPlayer.id};
        const otherUNOPlayer = {cards: [], id: otherPlayers[0].id};
        const controller = UNOAreaControllerWithProp({
            status: 'IN_PROGRESS', 
            players: [ourUNOPlayer, otherUNOPlayer],
        });
        expect(controller.ourDeck).toEqual([{color: 'Blue', rank: 1}, {color: 'Red', rank: 2}])
    });
    ...
    it('should throw an error if our player isnt in the game', () => {
        const otherUNOPlayer: UNOPlayer = {cards: [{color: 'Blue', rank: 1}, {color: 'Red', rank: 2}], id: otherPlayers[0].id};
        const anotherUNOPlayer = {cards: [], id: otherPlayers[1].id};
        const controller = UNOAreaControllerWithProp({
            status: 'IN_PROGRESS', 
            players: [otherUNOPlayer, anotherUNOPlayer],
  ...
  describe('othersCards', () => {
    it('should return a map of other players ids and amount of cards', () => {
        const ourUNOPlayer = {cards: [], id: ourPlayer.id};
        const otherUNOPlayer: UNOPlayer = {cards: [{color: 'Blue', rank: 1}], id: otherPlayers[0].id};
        const anotherUNOPlayer = {cards: [], id: otherPlayers[1].id};
...
describe('[] _updateFrom', () => {
    describe('if the game is in progress', () => {
        ...
        beforeEach(() => {
            ourUNOPlayer = {cards: [{color: 'Blue', rank: 1}, {color: 'Red', rank: 2}], id: ourPlayer.id};
            otherUNOPlayer = {cards: [{color: 'Yellow', rank: 1}, {color: 'Red', rank: 5}], id: otherPlayers[0].id};
            anotherUNOPlayer = {cards: [{color: 'Green', rank: 1}, {color: 'Green', rank: 2}], id: otherPlayers[1].id};
            finalUNOPlayer = {cards: [{color: 'Blue', rank: 1}, {color: 'Red', rank: 2}], id: otherPlayers[2].id};
         ...
         it('should emit a drawDeckChanged event with new deck', () => {
            ...
            if (deckChangedCall)
              expect(deckChangedCall[1]).toEqual([
                {color: 'Green', rank: 1}
            ]);
         });
         ...
         it('should emit an ourDeckChanged event if our players deck changed', () => {
            const model = controller.toInteractableAreaModel();
            const newDeckWithPlayer: UNOPlayer = {cards: [{color: 'Green', rank: 7}], id: ourPlayer.id};
            assert(model.game);
            const newModel: GameArea<UNOGameState> = {
                ...model,
                game: {
            ...
            if (ourDeckChangedCall) expect(ourDeckChangedCall[1]).toEqual([{color: 'Green', rank: 7}]);
         });
         ...
         it('should emit an otherCardsChanged event if other cards changed', () => {
            ...
            const newMap: Map<string, number> = new Map<string, number>([[otherPlayers[0].id, 0],
            [otherPlayers[1].id, 2], [otherPlayers[2].id, 2]]);
            assert(model.game);
            const newModel: GameArea<UNOGameState> = {
                ...model,
                game: {
         ...
         it('not emit an otherCardsChanged event if other cards didnt change', () => {
            const model = controller.toInteractableAreaModel();
            assert(model.game);
            const othersMap: Map<string, number> = new Map<string, number>([[otherPlayers[0].id, 2],
                [otherPlayers[1].id, 2], [otherPlayers[2].id, 2]]);
