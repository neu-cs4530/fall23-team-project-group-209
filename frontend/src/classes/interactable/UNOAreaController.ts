...
export default class UNOAreaController extends GameAreaController<UNOGameState, UNOEvents> {
  ...
  private _getPlayerDeck(id: string): Card[] | undefined {
     ...
     if (allPlayers) {
     const thePlayer: UNOPlayer | undefined = allPlayers.find(occupant => occupant.playerID === id)
     if (thePlayer) {
  ...
  get player1() : PlayerController | undefined {
    ...
    if (player1) {
      return this.occupants.find(occupant => occupant.id === player1.playerID)
    }
  ...
  get player2() : PlayerController | undefined {
    ...
    if (player2) {
      return this.occupants.find(occupant => occupant.id === player2.playerID)
    }
  ...
  get player3() : PlayerController | undefined {
    ...
    if (player3) {
      return this.occupants.find(occupant => occupant.id === player3.playerID)
    }
  ...
  get player4() : PlayerController | undefined {
    ...
    if (player4) {
      return this.occupants.find(occupant => occupant.id === player4.playerID)
    }
  ...
  get othersCards(): Map<string, number> | undefined {
    //create new map
    let playersAndCards: Map<string, number> = new Map<string, number>;
    ...
    if (allPlayers) {
      ...
      allPlayers.forEach((player) => {
        if (player.playerID !== ourPlayer) {
          playersAndCards.set(player.playerID, player.cards.length);
        }
      })
      } else {
        return undefined;
  ...
  get whoseTurn(): PlayerController | undefined {
    ...
    return this.occupants.find(occupant => occupant.id === player!.playerID)
    } else {
      return undefined;
    }
  }
  ...
  private _compareCards(pile1: Card[] | undefined, pile2: Card[] | undefined): boolean {
    if (!pile1 && !pile2) {
      ...
      return (pile1!.length === pile2!.length) && pile1!.every((card, index) => card === pile2![index]);
    }
  }
   ...
   */
  private _compareOrder(order1: PlayerController[] | undefined, order2: PlayerController[] | undefined): boolean {
    if (!order1 && !order2) {
      return true;
   ...
   */
  private _compareOtherDecksMap(others1: Map<string, number> | undefined , others2: Map<string, number> | undefined): boolean {
    if (!others1 && !others2) {
      return true;
    } else if ((!others1 && others2) || (others1 && !others2)) {
      return false;
    } else {
      let sameSize: boolean =  others1!.size === others2!.size;
      let sameKeyVal: boolean = true;
      ...
      for (let player in others1!.keys()) {
